            ...
            ->children()
                ->arrayNode('ssr')
                    ->children()
                        ...
                        ->end()
                        ->scalarNode('url')
                            ->info('The URL of the server-side rendering (SSR) server.')
                ...
                ->arrayNode('csrf_cookie')
                    ->children()
                        ->integerNode('expire')
                            ->info('Number of seconds after which the CSRF token cookie expires. . Set to 0 for a session cookie.')
                            ->defaultValue(0)
                        ->end()
                        ->scalarNode('path')
                        ...
                        ->end()
                        ->scalarNode('secure')
                            ->info('Indicates whether the cookie should only be transmitted over HTTPS.')
                            ->defaultTrue()
                        ->end()
                        ->scalarNode('sameSite')
